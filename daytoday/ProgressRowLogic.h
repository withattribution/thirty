//
//  ProgressRowLogic.h
//  daytoday
//
//  Created by pasmo on 10/15/13.
//  Copyright (c) 2013 Submarine Rich, LLC. All rights reserved.
//

#import "Intent+D2D.h"
#import "ChallengeDay+D2D.h"

#import <Foundation/Foundation.h>

typedef NS_ENUM(NSInteger, DTProgressRowEndStyle) {
    DTProgressRowEndFlat,
    DTProgressRowEndFlatLeft,
    DTProgressRowEndFlatRight,
    DTProgressRowEndBothRounded,
    DTProgressRowEndUndefined
};

typedef NS_ENUM(NSInteger, DTProgressRowTemporalStatus) {
    DTProgressRowPast,
    DTProgressRowCurrent,
    DTProgressRowFuture,
    DTProgressRowTemporalStatusUndefined
};

@interface ProgressRowLogic : NSObject

@property (nonatomic,retain) NSArray *row;
@property (nonatomic,retain) NSSet   *rowDateSet;
@property (nonatomic,retain) Intent *rowIntent;

//@property(nonatomic,readonly) NSInteger DTProgressRowEndStyle;
//@property(nonatomic,readonly) NSInteger DTProgressRowTemporalStatus;

- (id)initWithRow:(NSArray*)weekRow withIntent:(Intent *)intent;
- (DTProgressRowTemporalStatus)temporalStatusForRow;
- (DTProgressRowEndStyle)endStyleForRow;

@end
